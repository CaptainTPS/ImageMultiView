...
ccDBRoot::ccDBRoot(ccCustomQTreeView* dbTreeWidget, QTreeView* propertiesTreeWidget, QObject* parent) : QAbstractItemModel(parent)
{
	...
	m_storeRGBDImage = new QAction("Store all", this);
	m_addVideoGroup = new QAction("Add video files", this);
	...
	//tps
	connect(m_storeRGBDImage,					SIGNAL(triggered()),								this, SLOT(storeRGBDImage()));
	connect(m_addVideoGroup,					SIGNAL(triggered()),								this, SLOT(addVideoGroup()));
...
void storeCCImage(ccImage* img, QString basepath){
	ImageFileFilter imgFilter;
	//QString basepath("D:\\captainT\\project_13\\ImageMultiView\\Build\\qCC\\Debug");
	QString adder('\\');
	QString filename;
	...
	adder += img->getName();
	filename = basepath + adder;
	QString completeFileName(filename);
	if (QFileInfo(filename).suffix().isEmpty())
		completeFileName += QString(".%1").arg(imgFilter.getDefaultExtension());
	imgFilter.saveToFile(img, completeFileName, parameters);
}

void ccDBRoot::storeRGBDImage(){
	//output location
	QSettings settings;
	...
	QString basepath = QFileDialog::getExistingDirectory(0, "Save file", fullPathName);

	QModelIndexList selectedIndexes = m_dbTreeWidget->selectionModel()->selectedIndexes(); 
	ccHObject* item = static_cast<ccHObject*>(selectedIndexes[0].internalPointer());

	RGBDImage* img = ccHObjectCaster::ToRGBDImage(item);
	storeCCImage(ccHObjectCaster::ToImage(img), basepath);

	for (int i = 0; i < img->getChildrenNumber(); i++)
	{
		ccImage* child = ccHObjectCaster::ToImage(img->getChild(i));
		storeCCImage(child, basepath);
		for (int j = 0; j < child->getChildrenNumber(); j++)
		{
			ccImage* c_child = ccHObjectCaster::ToImage(child->getChild(j));
			storeCCImage(c_child, basepath);
		}
	}
	ccLog::Print(QString("store all done"));
}

void ccDBRoot::addVideoGroup(){
	...
	QModelIndex index = m_dbTreeWidget->indexAt(m_contextMenuPos);
	ccHObject* newGroup = new ccHObject("VIDEO");
	if (index.isValid())
	{
		ccHObject* parent = static_cast<ccHObject*>(index.internalPointer());
		if (parent)
	...
	for (int i = 0; i < files.size(); i++)
	{
		QString str = files[i];
		img->load(str);
		ccImage *c = new ccImage(*img);
		c->setName(QString::number(newGroup->getChildrenNumber()));
		newGroup->addChild(c);
	}
...
void ccDBRoot::showContextMenu(const QPoint& menuPos)
{
	...
	if (index.isValid())
	{
		...
		if (selCount)
		{
			...
			for (int i = 0; i < selCount; ++i)
			{
				...
				if (!item->isA(CC_TYPES::HIERARCHY_OBJECT))
				{
					...
					if (selCount == 1)
					{
						...
						else if (item->isA(CC_TYPES::RGBD_IMAGE))
						{
							isRGBDImage = true;
						}
					}
			...
			if (isRGBDImage)
			{
				menu.addAction(m_storeRGBDImage);
				menu.addAction(m_addVideoGroup);
				menu.addSeparator();
			}
