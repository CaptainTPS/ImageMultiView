...
void ccPropertiesTreeDelegate::fillModel(ccHObject* hObject)
{
	...
	else if (m_currentObject->isKindOf(CC_TYPES::IMAGE))
	{
		...
		if (m_currentObject->isKindOf(CC_TYPES::RGBD_IMAGE))
		{
			fillWithRGBDImage(ccHObjectCaster::ToRGBDImage(m_currentObject));
		}
	}
...
void ccPropertiesTreeDelegate::fillWithRGBDImage(RGBDImage* _obj)
{
	assert(_obj && m_model);
	...
	appendRow(ITEM("Depth Image"), ITEM(IDG));

	appendWideRow(PERSISTENT_EDITOR(OBJECT_RGBD_IMAGE));
	...
	const QModelIndex &index) const
{
	...
	case OBJECT_RGBD_IMAGE:
	{
		DealDepthDlg* ddd = new DealDepthDlg(parent);
		outputWidget = ddd;
	}
	break;
	case TREE_VIEW_HEADER:
...
void ccPropertiesTreeDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
	...
	case OBJECT_RGBD_IMAGE:
	{
		DealDepthDlg *ddd = qobject_cast<DealDepthDlg*>(editor);
		if (!ddd)
			return;

		ddd->connectOBJ(m_currentObject);
		break;
	}
	case OBJECT_HISTORY_MATRIX_EDITOR:
	{
